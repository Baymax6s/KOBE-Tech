...
**理由：** エンティティ名（`Reply`）と種別値（`comment` / `question` / `answer`）を分けたので、UI チップやタグの「コメント / 質問 / 回答」と種別値が一致して齟齬がない。コード上も `reply.kind === "comment"` で読みやすい。また、`type`ではなくて`kind`を採用したのは`ts`において名前の衝突が起きにくいと考えた耐衝突が起きにくいと考えたため。
...
| メソッド | パス                                  | 説明                          | 認証 |
| -------- | ------------------------------------- | ----------------------------- | ---- |
| GET      | `/api/v1/articles/:articleId/replies` | 記事の返信全件を取得          | 不要 |
| POST     | `/api/v1/articles/:articleId/replies` | コメント / 質問 / 回答を投稿  | 必須 |
